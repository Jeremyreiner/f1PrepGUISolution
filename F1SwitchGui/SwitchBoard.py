...
pro = r"C:\Users\reine\OneDrive\WorkPC\f1pygui\F1SwitchGui\switch.csv"
work = r"C:\Users\Jeremy\OneDrive\WorkPC\f1pygui\F1SwitchGui\switch.csv"
...
sgTextWidthHeight = (10, 2)
sgBorderWidth = 0

requests = [] #READ FROM CSV AND RETURN LIST
guiLayout = []
toggleMap = {}

with open(work, newline='') as csvfile:
    ...
    for row in csvreader:
        ...
        for i in range(len(row)):
            ...
            if row[i].find(','):
                row[i] = row[i].replace(',','').title()
        
        requests.extend(row)
requests.sort()

TurnAllOn = [
    sg.Text('All On', size=sgTextWidthHeight),
    
    sg.Button(image_data=toggle_btn_off, key='-TOGGLE-GRAPHIC-All-ON-', 
        button_color=(sg.theme_background_color(), sg.theme_background_color()), 
        border_width=sgBorderWidth)
]

TurnAllOff = [
    sg.Text('All Off', size=sgTextWidthHeight),
            
    sg.Button(image_data=toggle_btn_on, key='-TOGGLE-GRAPHIC-All-OFF-', 
        button_color=(sg.theme_background_color(), sg.theme_background_color()), 
        border_width=sgBorderWidth )
]

for x in requests:
    toggleMap[x + "-graphic_off-"] = True #true means graphic is off false means graphic is on
    
    toggle = [
        
            sg.Text(f'{x}', size=sgTextWidthHeight),
            
            sg.Button(image_data=toggle_btn_off, key=f'-TOGGLE-GRAPHIC-{x}-', 
                button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0)
        ]

    guiLayout.append(toggle)

toggleMap["AllOn"], toggleMap["AllOff"] = True, False

guiLayout.append(TurnAllOn)

guiLayout.append(TurnAllOff)
