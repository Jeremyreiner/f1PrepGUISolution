
from ctypes import sizeof
from msilib.schema import Font, TextStyle
import PySimpleGUI as sg
from guiValidationFunctions import *
import serial.tools.list_ports
import pyautogui
from tkinter import ttk
from tkinter import *
popAnim = b''

selection = [str(x)[str(x).find('cu.')+3:-6] for x in list(serial.tools.list_ports.comports())]
screen_width, screen_height= pyautogui.size()
clmnSize = (screen_width // 5, screen_height // 5)
inputWidth = round(clmnSize[0] * .09)
buttonWidth = round(clmnSize[0] * .01)
buttonTxt = "📄"
playBtn = "⏯️"
stopBtn = "⏸"
input_btn_height = 1
inputPadding = ((3, 6))
screen_hight_used = screen_height - ((clmnSize[1] // 2) + (clmnSize[1] - 100))
textboxHeight = (screen_height - screen_hight_used) // 9

col1 = [
    [sg.Text('Serial Port')], 
    [sg.Combo(selection,default_value=selection[0],size=(
        (inputWidth + buttonWidth), input_btn_height),readonly=True, key='-SERIAL_PORT-', pad=inputPadding)],
    [sg.Text('Host_Ip')], 
    [sg.Input(size=(inputWidth + buttonWidth, input_btn_height), key='-HOST_IP_INPUT-')],
    [sg.Button('Network Settings', pad=(2, 35), key='-NETWORK_SETTINGS-')]
]
col2 = [
    [sg.Text('Image Path')], [sg.Input(size=(inputWidth, input_btn_height), key='-IMAGE-'),
                              sg.FileBrowse(buttonTxt,buttonTxt, pad=inputPadding)],
    [sg.Text('Embedded_src_Path')], [sg.Input(size=(inputWidth, input_btn_height),
                                              key='-EMBEDED_SRC-'), sg.FileBrowse(buttonTxt,buttonTxt)],
]
col3 = [
    [sg.Text('farmserverfillsrc')], [sg.Input(size=(inputWidth, input_btn_height),
                                              key='-FARMSERVER-'), sg.FileBrowse(buttonTxt, pad=inputPadding)],
    [sg.Text('farmserverf1installersrc')], [sg.Input(size=(inputWidth, input_btn_height),
                                                     key='-FARMSERVERF1INSTALLER-'), sg.FileBrowse(buttonTxt, pad=inputPadding)],
    [sg.Text('pathtodbfile')], [sg.Input(size=(inputWidth, 1),
                                         key='-DB_FILE-'), sg.FileBrowse(buttonTxt)]
]
col4 = [
    [sg.Checkbox('', key='-IMPORTDBBOOL-'),sg.Text('import database',size=(inputWidth, input_btn_height))],
    [sg.Text('nand_src_path')], [sg.Input(size=(inputWidth, input_btn_height), key='-NAND_SRC_PATH-'),
                                 sg.FileBrowse(buttonTxt, pad=inputPadding)],
    [sg.Text('dbfilesrc')], [sg.Input(size=(inputWidth, input_btn_height),
                                      key='-DB_FILE_SRC-'), sg.FileBrowse(buttonTxt)]
]
col5 = [
    [sg.vbottom(sg.Button('Save Settings',pad=(round(clmnSize[1] * .1) , round(clmnSize[1] * .6)), key='-SAVE-'),)]
]

row2Col1 = [
    [sg.Text('Serial Number')
     ], [sg.Input(size=(inputWidth, input_btn_height), pad=inputPadding, key='-SN-')],
    [sg.Text("TO_RTS_PORT")], [sg.Input(size=(inputWidth, input_btn_height), pad=inputPadding,  key='-TO_RTS-',)],
    [sg.Text("FROM_RTS_PORT")
     ], [sg.Input(size=(inputWidth, input_btn_height),  key='-FROM_RTS-')],
]
row2Col2 = [
    [sg.Text("som_desired_ip")], [sg.Input(size=(inputWidth, input_btn_height),
                                           pad=inputPadding, key='-SOM_DESIRED_IP-')],
    [sg.Text('f1_unit_prep_final_ip_config')], [sg.Combo(
        ('DHCP', 'Static'),default_value='DHCP', size=(inputWidth, input_btn_height),readonly=True, key='-DHCP-', pad=inputPadding)],

]
row2Col3 = [[]]
row2Col4 = [[]]
row2Col5 = [
    [sg.Button(playBtn,font=("",36),key='-CONTINUE-', visible=True),
        sg.Button(stopBtn,font=("",36), key='-STOP_LOG-',visible=False)],
    [sg.Button('Open Log Folder', key='-OPEN_LOG_FOLDER-', visible=True)],
]


# -------ROW DESIGN INSIDE OF window1----------------
row1 = [[
    sg.Frame('Settings',
             [[
                 sg.Col(col1, p=0, size=((clmnSize[0]), (clmnSize[1]))), sg.Col(
                     col2, p=0, size=((clmnSize[0]), (clmnSize[1]))),
                 sg.Col(col3, p=0, size=((clmnSize[0]), (clmnSize[1]))), sg.Col(
                     col4, p=0, size=((clmnSize[0]), (clmnSize[1]))),
                 sg.Col(col5, p=0, size=((clmnSize[0]), (clmnSize[1])))
             ]])
]]
row2 = [[
        sg.Col(row2Col1, p=0, size=((clmnSize[0]), (clmnSize[1]))), sg.Col(
            row2Col2, p=0, size=((clmnSize[0]), (clmnSize[1]))),
        sg.Col(row2Col3, p=0, size=((clmnSize[0]), (clmnSize[1]))), sg.Col(
            row2Col4, p=0, size=((clmnSize[0]), (clmnSize[1]))),
        sg.Col(row2Col5, p=0, size=((clmnSize[0]), (clmnSize[1])), pad=(20)),
        ]]
row3 = [
    [
        sg.Text('Status Notes'),
        sg.Image(data=popAnim, enable_events=True, visible=False ,pad=((screen_width *.769,0), 0), key='-LOADING-', right_click_menu=['UNUSED', ['Exit']]),
    ],
    [sg.Output(size=(round(screen_width), textboxHeight), key='-LOG-')],
    [sg.ProgressBar(100, orientation='h', size=(round(screen_width), (round(screen_height / 20))), key='-PROGRESSBAR-')],
]
# -------ROW DESIGN INSIDE OF window1----------------

def Make_Win1():
    layout = [
        [
            [row1, row2, row3]
        ]
    ]
    win1 = sg.Window("Dialog Title", layout,resizable=True, finalize=True)
    win1.bind('<Configure>', "-CONFIGURE-") # binding resize element to window

    return win1